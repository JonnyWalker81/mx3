...
void
Http::get(const string& url, const std::experimental::optional<std::unordered_map<std::string, std::string>> & headers, function<void(HttpResponse)> m_cb) {
    m_http->get(url, headers, make_shared<Http::Request>(std::move(m_cb), m_cb_thread) );
}

void Http::post(const string &url, const string &body, const std::experimental::optional<std::unordered_map<std::string, std::string>> & headers, function<void(HttpResponse)> cb)
{
    m_http->post(url, body, headers, make_shared<Http::Request>(std::move(cb), m_cb_thread));
}
